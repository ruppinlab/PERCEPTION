...
We have demonstrated the application of PERCEPTION on three clinical trial datasets for different cancer types and its usability to predict the treatment response and ability to detect the development of patients’ resistance during treatment. The scripts (Step2_Figure1.Rmd , Step3_Figure3.Rmd, Step5_Figure5_and_Extended_Figure9_10.Rmd, Step6_ExtendedFigure4-6.Rmd, Step7_ExtendedFigure1.Rmd) described in Section 1 were designed to address the specific aspects associated with individual datasets. Therefore, to provide guidance to the users on how PERCEPTION could be used for new clinical trial datasets, we have generated a new script named <b>“Running_PERCEPTION_for_new_dataset.Rmd”</b>. This script will provide the fundamental functionality of PERCEPTION in predicting treatment response using a patient's scRNA-seq expression data for particular drugs or combination therapies. 
<br><br>
<b>PS:</b> It should be emphasized that the application of PERCEPTION is not limited to the aforementioned script. Researchers can utilize the PERCEPTION model to explore various aspects of their studies. Section 1 has already provided examples of this, where we employed PERCEPTION to analyze three different clinical trial datasets and addressed several aspects associated with individual datasets.
...
In the script, using the sample datasets, we have demonstrated how PERCEPTION could be use for two drugs. The test dataset and the output figures are provided in the Test directory. The Test directory has two subdirectories (i) Test_data and (ii) Test_figures. 

(i) Test_data directory contains the test dataset that was used as input as clinical response data for the /Tools/Running_PERCEPTION_for_new_dataset.Rmd script.
(ii)Test_figures directory contains the figures generated by running /Tools/Running_PERCEPTION_for_new_dataset.Rmd script with test datasets.
