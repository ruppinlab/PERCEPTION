...
# Set working directory to the directory where "project_SCPO_submission" is cloned.
...
ggsave(tsne_together_gefitinib, filename = '../../Figures/Figure1_erlotinibv1.png', height=2.5, width=5.6)
```
<!-- Figure 1B-C -->
<!-- Performance of our models in cross validation -->
```{r}
drugNames_fda_approved_used=drugNames_fda_approved[na.omit(match(approved_drugs_for_cancer,
                                          stripall2match(drugNames_fda_approved)))]
approved_drugs_for_cancer_in_screen=unique(onTarget$secondary_screen_drugAnnotation$CommonName[na.omit(match(approved_drugs_for_cancer, stripall2match(onTarget$secondary_screen_drugAnnotation$CommonName)))])
write.csv(approved_drugs_for_cancer_in_screen,
          '../../Data/approved_drugs_for_cancer_in_screen.csv')
```

```{r}
fileNames_ofInterest=list.files('Data/cancer_approved_drugs/')[grep('RDS',list.files('Data/cancer_approved_drugs'))]
# performance_extended_grid=lapply(fileNames_ofInterest, get_performance)
performance_extended_grid=readRDS('Data/model_performances.RDS')
drugNames_fda_approved=strsplit_customv0(fileNames_ofInterest, infunc_split_by = '_', retreving_onject_id = 1)
names(performance_extended_grid)=drugNames_fda_approved
