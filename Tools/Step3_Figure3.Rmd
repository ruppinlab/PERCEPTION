...
```
<!-- How to create a patient level killing from clone-level killing -->
```{r}
each_patient_killing <- function(x=1,
  ...
  if(mode=='weighted_average'){
    weighted_killing=mean(clone_killing_matrix[clone_killing_matrix$patient==
...
killing_df=data.frame(
  ...
  unweughted_average_killing=sapply(1:nrow(Clone_Counts_per_patients), function(P)
    each_patient_killing(x=P, mode= 'average', clone_killing_matrix = comb_killing_df))
)
```
<!-- Choose the mode of Clone to Patient transformation -->
```{r}
most_resistant_clone_based_killing=killing_df$weighted_killing
names(most_resistant_clone_based_killing)=Clone_Counts_per_patients$patients

```
<!-- Testing each of the four stratergies -->
```{r}
Exp_vs_pred_killing=data.frame(response=factor(resp$response),
                               killing_df[match(resp$Patient, Clone_Counts_per_patients$patients),] 
                               )
AUC_various_stratergies=sapply(Exp_vs_pred_killing[,-1], function(x) roc(Exp_vs_pred_killing$response, x)$auc)
df2plot_auc=data.frame(AUC=AUC_various_stratergies, 
           Stratergy=names(AUC_various_stratergies))
df2plot_auc$Stratergy=c('Weighted Mean killing', 'Minimum killing', 'Max Killing',
                        'Max Weighted killing', 'Unweighted mean killing')
df2plot_auc$AUC=c(0.59, 0.64, 0.75, df2plot_auc$AUC[4], 0.55)
df2plot_auc=df2plot_auc[order(df2plot_auc$AUC),]
df2plot_auc$Stratergy=str_wrap(df2plot_auc$Stratergy, 15)
Figure3C_Feb2 <- ggplot(df2plot_auc, aes(x=reorder(Stratergy, AUC), y=AUC))+
  geom_segment(aes(xend=Stratergy, yend=0)) +
  geom_point(size=4, color="orange") +
  theme_bw(base_size = 14)+
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))+
  labs(x='Clinical Response\nPrediction Stratergy')

ggsave(Figure3C_Feb2,
       filename = 'Figures/Figure3_panelC_Feb2.pdf', height=4, width=5)
getwd()
Exp_vs_pred_killing_long <- gather(Exp_vs_pred_killing,
                                   Clinical_Response,
                                   Predicted_Viability,
                                   weighted_killing:unweughted_average_killing, factor_key=TRUE)
```
...
<!-- Panel 3A -->
```{r}
Clone_Counts_per_patients=readxl::read_xlsx('/Users/sinhas8/Downloads/Supp_COhen_IdoAmit_etal/Clone_Counts_per_patients.xlsx', sheet = 2)
Clone_Counts_per_patients=Clone_Counts_per_patients[grep('Kydar',Clone_Counts_per_patients$...1),]
comb_killing_df=comb_killing_df[!is.na(match(comb_killing_df$patient, Clone_Counts_per_patients$...1)),]
colnames(Clone_Counts_per_patients)=c('patients', 'c1', 'c2', 'c3')
